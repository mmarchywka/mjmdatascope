#ifndef MJM_COLLECTIONS_H__
#define MJM_COLLECTIONS_H__

// #warning " moved taxonomy stuff out 2021-01-25 include mjm_taxonomy_stuff AFTER this "  
#include "mjm_globals.h"

// this is circular uses ragged lol
// for ragged now 
...
// a mess for now... 
typedef StrTy _Word;
typedef std::vector<_Word> _Line;
class _ActualLine : public _Line
{
public:
const void dump(OsTy & os) const
{
MM_LOOP(ii,(*this)) { os<<(*ii)<<" "; }
} // dump 
const StrTy dump(const IdxTy flags=0) const
...
// this really fudds everything up... shopuld use a setter
Line & operator[] (const IdxTy i )   { return m_lines[i]; } 
void set(const Line & l , const IdxTy i )   {  m_lines[i]=l; } 

void  hdrs(const IdxTy hdrs )  { m_hdrs=hdrs; } 
IdxTy hdrs() const { return m_hdrs; } 
...
StrTy debug_line(const IdxTy i) const
{
if (i>size()) return StrTy();
Ss ss; 
const IdxTy len=m_lines[i].size();
...
/////////////////////////////////////////////////////////////
void load_params(const StrTy & src,const StrTy & sin, const IdxTy flags)
{
BaseParams kvp(sin);
StrTy cmd="";
StrTy sep=" ";
IdxTy nwords=1;
const StrTy name="";
bool load_string=false; // (cmd=="string-ragged");
bool read_code=false; // (cmd=="read-code");
bool load_parsed=!false; // ((flags&1)==0);
//bool tabsep=((flags&2)!=0);
bool ignore_hash=false; // ((flags&4)!=0);
bool debug=false; // ((flags&8)!=0);
//bool csv=((flags&16)!=0);
bool pdftext=false; // ((flags&32)!=0);
bool ignore_2delims=false; // ((flags&64)!=0);
//MM_ERR(" cmd_read_ragged from "<<fn<<" to "<<name<<" flags "<<flags)
MM_ERR(MMPR(__FUNCTION__)<<MMPR4(cmd,src,name,sep)<<MMPR3(debug,pdftext,ignore_2delims)<<MMPR3(load_string, load_parsed,ignore_hash))
Myt & r=*this; // rm[name];
...
r.sep(sep);
if (load_string) r.load_from_string(src,debug);
else if (load_parsed) r.load(src,debug);
else r.load_lines(src,debug);
MM_ERR(MMPR2(r.size(),name))

} // load_params
void load_canned(const StrTy & cmd, const StrTy & fn, const IdxTy flags, const IdxTy nwords)
{
...
void set_eol(const IdxTy flags=0) 
{
if ((flags&1)==1) m_eol="\r\n";

} // set_eol 
bool load_params(const IdxTy n, const IdxTy ty, const StrTy& s="")
{
sep(s);
//if (m_eol=="\r\n") li.cr_or_lf(true);
switch (ty)
{
case 0: {splitter(n);break;}
case 1: {code_splitter(n);break;}
case 2: {kvp_splitter(n);break;}
case 3: {idiom_splitter(n);break;}
default:
MM_ERR(" bad params for ragged "<<MMPR3(n,ty,s))
return false;  
} // switch
...
void eol(const StrTy & s) { m_eol=s;} 
void code_splitter(const IdxTy & s) { m_splitter=s+CommandInterpretter::CODE_SPLIT_BASE; } 
void kvp_splitter(const IdxTy & s) { m_splitter=s+CommandInterpretter::KVP_SPLIT_BASE; } 
void idiom_splitter(const IdxTy & s) { m_splitter=s+CommandInterpretter::IDIOM_SPLIT_BASE; } 
...
{
// the incremental approach wrecks min and max word counts per line,,,, 
if (m_lines.size()==0) {  m_lines.push_back(Line());  return *this; } 
if (m_lines.back().size()==0) return *this; 
m_lines.push_back(Line());
return *this; 
}
...
Myt& operator<<(const char *  word) {Ss ss; ss<<StrTy(word); return (*this)<<ss.str(); }
Myt& operator<<(const unsigned char *  word) {Ss ss; ss<<StrTy((char *)word); return (*this)<<ss.str(); }
...
CommandInterpretter * dynamic_split()
{
add_ci();
...
const void dump_line(OsTy & os,const Line  & line ) const
{
MM_LOOP(ii,line) { os<<(*ii)<<" "; }
} // dump 
const StrTy dump_line(const IdxTy i, const IdxTy flags=0) const
{ Ss ss; dump_line(ss,(*this)[i]); return ss.str(); }  
...
//{const char c=p[i];if ( need_esc(c,cs)) { ss<<'\\'; }   ss<<c; } 
bool need_esc(const char c ) const  { return ((c==' ')||(c=='"')); }  
bool need_esc(const char c,const char cs ) const  { return ((c==cs)||(c=='"')); }  
// this logic is all wrong for cr lf etc crap 
// TODO FIXME 
template <class Ty> 
bool stream_esc(Ty & ss, const char c,const char cs ) const  { 
//return ((c==cs)||(c=='"')); 
//const char c=p[i];
const bool x= ( need_esc(c,cs)) ;
if (x) { ss<<'\\'; }   
ss<<c;  
return x; 
}  
...
const bool use_none=((flags&64)!=0);
const bool use_safe=((flags&128)!=0);
const bool print_flags=((flags&256)!=0);
const IdxTy safe_flags=((flags>>8)&15);
if (print_flags) {
MM_ERR(MMPR(flags)<<MMPR4(use_space,add_seq,add_quote,add_escapes)
<<MMPR4(use_s,debug_parse,use_none,use_safe)<<MMPR(safe_flags))
}
...
const StrTy alts=SafeString((*jj),cs,safe_flags);
//const bool unsafe=(alts!=(*jj)); 
if ( add_sep) ss<<sep;
...
for(IdxTy i=0; i<l1; ++i ) 
//{const char c=p[i];if ( need_esc(c,cs)) { ss<<'\\'; }   ss<<c; } 
{const char c=p[i];stream_esc(ss,c,cs);  } 
}
...
StrTy SafeString(const StrTy & s, const char csep, const IdxTy flags=0) const
{
const bool remove_cs=Bit(flags,0);
const bool quote_unsafe=Bit(flags,1);
const bool uscore_unsafe=Bit(flags,2);
//StrTy x;
IdxTy unsafe=0;
...
if (c==csep) {++unsafe;  
if (uscore_unsafe) { ss<<"_"; } 
else if ( !remove_cs) { ss<<"\\"; ss<<c;}  ++i;  continue; }
if ( c=='\r' ){ ++unsafe; 
if (uscore_unsafe) { ss<<"_"; }  else 
ss<<"\\r";   ++i; continue; } 
if ( c=='\n' ) { ++unsafe; 
if (uscore_unsafe) { ss<<"_"; }  else 
ss<<"\\n";   ++i; continue; } 
...
++unsafe; 
if (uscore_unsafe) { ss<<"_"; }  else 
// https://stackoverflow.com/questions/21416425/printing-zero-padded-hex-with-stdcout
...
if (quote_unsafe) if (unsafe) return StrTy("\"")+s+StrTy("\""); 
// for size exceeding stack size... 
/*
const char * cc= new char[5*(len+1)];
IdxTy pc=0;
IdxTy i=0;
while (p[i]!=0)
{
const char c=p[i];
if ( c==csep ) { cc[pc]='\\'; ++pc; cc[pc]=c; ++pc;  ++i; continue; } 
if ( c=='\r' ) { cc[pc]='\\'; ++pc; cc[pc]='r'; ++pc;  ++i; continue; } 
if ( c=='\n' ) { cc[pc]='\\'; ++pc; cc[pc]='n'; ++pc;  ++i; continue; } 
...
// TODO FIXME add a latex output option 
class _latex_out_options
{
public:
_latex_out_options(const StrTy & x) { Init();  m_s=x; }
_latex_out_options() { Init();  }
const IdxTy pad_size() const { return m_pad_size;}
IdxTy load(const StrTy & fn, const IdxTy flags=0)
{
MM_ERR(" loading desc rile "<<MMPR(fn))
std::ifstream ifs(fn);
IdxTy line=0;
const IdxTy bufsz=1<<16;
char b[bufsz];
b[0]=0;
IdxTy dest=0;
while (!ifs.eof())
{
if (ifs.bad()) {MM_ERR(" stream bad bignoring "<<MMPR4(fn,bufsz,line,b)) } 
ifs.getline(b,bufsz);
if (strncmp(b,"%\\input{",8)==0) {dest=1; continue; } 
if (dest==0) m_header+=StrTy(b)+"\n";
else if (dest==1) m_trailer+=StrTy(b)+"\n";
++line;
b[0]=0;
} // eof 
MM_ERR(" loaded desc rile "<<MMPR3(fn,m_header.length(),m_trailer.length()))
return 0;
} // load
void Init()
{
...
StrTy m_header,m_trailer;

StrTy m_s;
IdxTy m_max, m_pad_size;
}; // _latex_out_options

typedef _latex_out_options latex_out_options;

StrTy dump_latex( const IdxTy flags=0,const latex_out_options &  loss=StrTy()) const
{
Ss ss;
IdxTy i=0;
IdxTy bi=0;
...
StrTy seps="&";
if (use_space) seps=" ";
if (use_s) seps=loss.m_s;
// added 2020-12-27 to bring arb sep out
if (use_s&&use_space) seps=m_sep;
StrTy quote="\"";
std::vector<StrTy> runnings;
const StrTy sep=seps;
const char cs=sep.c_str()[0];
MM_LOOP(ii,m_lines)
{
Ss rr;
if (add_breaks) if (bi==0) { ss<<loss.m_header; }  // s<<CRLF;
if (bi==0) if (add_headers) { MM_LOOP(jj,runnings) { 
...
if (i==hdrs()) if (add_headers) { ss<<"\\hline"<<CRLF; } 
...
if (debug_parse) ss<<MMPR(line.size()<<" ");
bool add_sep=false;
// this makes a mess of replay... 
//if (add_seq) {add_sep=true;  ss<<i; }
if (add_seq) {add_sep=true;  }
IdxTy debug_field_count=0;
MM_LOOP(jj,line)
{
if (debug_parse) { rr<<" *("<<debug_field_count<<")* "; } 
// TODO FIXME  this should quote or escape where needed
if ( add_sep) rr<<sep;
if (add_quote) rr<<quote;
if(add_escapes)
{
const IdxTy l1= (*jj).length();
const char * p=(*jj).c_str();
for(IdxTy i=0; i<l1; ++i ) 
//{const char c=p[i];if ( need_esc(c,cs)) { ss<<'\\'; }   ss<<c; } 
{const char c=p[i];stream_esc(rr,c,cs);  } 
}
else { rr<<(*jj); }  // TODO FIXME XXX need to escape 
if (add_quote) ss<<quote;
++field;
add_sep=true;
++debug_field_count;
...
if (add_padding)
{
const IdxTy llen=line.size();
for(IdxTy k=llen; k<loss.pad_size(); ++k ) rr<<" & ";
} // add_padding 


rr<<" \\\\"<<CRLF;
if (i<hdrs()) runnings.push_back(rr.str());
++bi;
if (add_seq) {ss<<i; }
ss<<rr.str();

if ((add_breaks)&&(bi>=loss.m_max)) { ss<<loss.m_trailer ;  bi=0;}  // s<<CRLF;
++i;
} 

if ((add_breaks)&&(bi>0)) { ss<<loss.m_trailer ;  bi=0;}  // s<<CRLF;
...
template <class Ty> void   append_generic_block( const Ty & b, const IdxTy nx, const IdxTy ny, const IdxTy flags)
{

const bool set_prec_hi=Bit(flags,0);
const bool set_prec_lo=Bit(flags,1);
const bool set_prec = set_prec_hi||set_prec_lo;
const bool transpose=Bit(flags,4);
IdxTy prec=16;
if ( set_prec_hi&&set_prec_lo )
{} // TODO FIXME use some member variable or something. 
else if (set_prec_hi) prec=16;
else prec=4;
const IdxTy nprec=prec;
...
{
Ss ss; 
if (set_prec) { ss<<std::setprecision(nprec); }
//if (b(j,i)>2) { MM_ERR(" danger "<<MMPR3(i,j,b(j,i))) } 
ss<<b(j,i);
l.push_back(ss.str());
} // j 
add(l);
...
Ss ss; 
if (set_prec) { ss<<std::setprecision(nprec); }
//if (b(i,j)>2) { MM_ERR(" danger "<<MMPR3(i,j,b(i,j))) } 
ss<<b(i,j); 
l.push_back(ss.str());
} // j 
add(l);
...
/////////////////////////////////
// iterators
class _word_iterator
{
public:
_word_iterator( Myt & r,const bool rev=false) : m_r(r) {Init(rev); }

Word & operator*() { return m_r[m_line][m_word]; }
const IdxTy line() const { return m_line; } 
const IdxTy lines() const { return m_sz; } 
const IdxTy word() const { return m_word; } 
const IdxTy words() const { return m_len; } 
 operator bool() const { return m_ok; } 
_word_iterator & operator++() { Inc(); return *this; } 
_word_iterator & operator--() { Dec(); return *this; } 
private:
void Init(const bool rev)
{
m_sz=m_r.size();
if (rev){  m_line=m_sz-1;  Prior(); return ; } 
m_line=0;
m_word=0;
Next();
} // Init
...
Myt & m_r;
IdxTy m_sz,m_len,m_line,m_word;
bool m_ok;
...
{
li.readline_ok(false);
const IdxTy eols=m_eol.length();
if (m_eol=="\r\n") li.cr_or_lf(true);
else if ( eols==1) li.use_alt_eol(m_eol.c_str()[0],true);
//li.use_alt_eol(',',true);
if (debug){
MM_ERR(MMPR3(m_sep,m_sep.length(),m_splitter))
}
if (m_sep.length()>0)
    li.set_split(m_splitter,m_sep.c_str()[0]);
    else li.set_split(m_splitter,' ');
//MM_ERR(" AFK "<<MMPR3(m_splitter, m_sep,m_sep.length()))
if (debug) { MM_ERR(" AFK "<<MMPR4(li.SPLITS(),m_splitter, m_sep,m_sep.length())) } 
}
...
// add properties or something?
// serialize etc in another class? 
// MEMBERS

Lines m_lines;
IdxTy m_hdrs, m_min_words, m_max_words;
StrTy m_sep;
StrTy m_eol;
IdxTy m_splitter;
bool m_ignore_hash;
...
StrTy m_name;
 ...
 class mjm_ragged_cursor;

class mjm_ragged_config
{
public:
typedef unsigned int IdxTy;
virtual IdxTy configure(mjm_ragged_cursor & r) =0; 
...
typedef mjm_ragged_cursor Myt;
typedef mjm_ragged_table Ragged;
typedef Ragged::Line Line;
typedef Ragged::Word Word;
typedef unsigned int IdxTy;
public:
mjm_ragged_cursor(): m_tgt(0) {Init(); } 
// could have a const version? 
//mjm_ragged_cursor(Ragged & r ): m_tgt(&r),m_ctgt(&r) {Init(); } 
//mjm_ragged_cursor(const Ragged & r ):m_tgt(0), m_ctgt(&r) {Init(); } 

mjm_ragged_cursor(const Ragged & r ): m_tgt(&r) {Init(); } 
//mjm_ragged_cursor(const Ragged & r ):m_tgt(0), m_ctgt(&r) {Init(); } 
// this fails with zero length lines... 
operator bool() const { return m_line<m_sz; } 
const IdxTy i() const { return m_line; } 
const IdxTy j() const { return m_word; } 
const Line & line() const { return (*m_tgt)[m_line]; } 
const Word & word() const { return (*m_tgt)[m_line][m_word]; } 
const Word & operator*() const { return word(); } 
//Line & line() { return (*m_tgt)[m_line]; } 
//Word & word() { return (*m_tgt)[m_line][m_word]; } 
Myt & operator++() { Inc(); return *this; } 
const IdxTy dline() const { return m_line-m_iline; } 
const IdxTy dword() const { return m_word-m_iword; } 
private:
...
if (!m_tgt) return;
++m_word;
if  (m_word>=m_len)  { m_word=0; ++m_line; } 
while (*this) 
{
m_len=(*m_tgt)[m_line].size();
if (m_len) return; 
++m_line;
} // this 
...
{
m_sz=m_tgt->size();
if (m_sz) m_len=(*m_tgt)[0].size();
while (m_len==0)
{
++m_line;
if (m_line>=m_sz) break;
m_len=(*m_tgt)[m_line].size();
} // m_len
...
//Thing *  m_tgt;
//const Ragged *  m_ctgt;
IdxTy m_line,m_word,m_len,m_sz;
IdxTy m_iline, m_iword;
}; // mjm_ragged_cursor
...
#if 0 // PROBABLY COMMENTS OUT THE TAX STFF

#error " don't include this the real code is in mjm_taxonomy_stuff.h"
