...
const dofmap_type & get_dofs() const { return m_dofs; } 
...
//{ 
//++i;
const auto & t=m_tri[i];// (*ii);
const vertex_type & v1=m_verticies[t.p1-1];
const vertex_type & v2=m_verticies[t.p2-1];
const vertex_type & v3=m_verticies[t.p3-1];
//v=Ti(t.p1-1,t.p2-1,t.p3-1);
switch(n) 
{
case (0) :{v=t.p1-1; coords=Tp(v1.x,v1.y); break; }
case (1) :{v=t.p2-1; coords=Tp(v2.x,v2.y); break; }
case (2) :{v=t.p3-1; coords=Tp(v3.x,v3.y); break; }
...
// for the datascope field sens the values are on the
// vertex line... doh 
const bool extra_vertex_data=Bit(flags,3);
MM_ERR(MMPR4(__FUNCTION__,save_old,total_area,write_R)<<MMPR2(extra_vertex_data,flags))
std::vector<dof_type>  temp_dofs;
if (!save_old) Clear(); 
...
if (lsz!=3)
{
if (extra_vertex_data)
{
const IdxTy ls=3; 
const IdxTy lf=lsz; 
// this is incredibly slow no matter how you do it now... 
// much better to make local array of dofs and assign later.. 
while (temp_dofs.size()<=(lf-ls)) temp_dofs.push_back(dof_type()); 
for(IdxTy idof=ls; idof<lf; ++idof)
{
//MM_ERR(MMPR3(idof,l[idof],(atof(l[idof].c_str()))))

temp_dofs[idof-ls].push_back(atof(l[idof].c_str()));
} // idof 

}else MM_ERR(" vertex line size wrong "<<MMPR2(i,l.size()))
if (lsz<2) return BAD; 
}
...
if (temp_dofs.size())
{
MM_ILOOP(i,temp_dofs.size())
{
Ss ss;
ss<<i;
m_dofs[ss.str()]=temp_dofs[i];
} // i 
} // temp_dofs.size()
...
void Compile()
{
MM_LOOP(ii,m_dofs) { (*ii).second.find_stats(); } 
