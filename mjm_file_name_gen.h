...
typedef std::map<StrTy,IdxTy> ExistsMap;
// API

public:
mjm_file_name_gen(): m_serial(0),m_mode(0)  {Init();}
~mjm_file_name_gen() {}

IdxTy nonexist(StrTy & fnout, const StrTy & fnin, const IdxTy flags=0 )
{ return NonExist(fnout,fnin,flags); }
...
IdxTy NonExist(StrTy & fnout, const StrTy & fnin, const IdxTy flags )
{ 
IdxTy n=0;
while (true)
{
const auto ii=m_exists.find(fnin);
if (ii!=m_exists.end()) n=(*ii).second+1;
m_exists[fnin]=n;
Ss ss; ss<<m_now<<"_"<<n<<"_"<<fnin;
fnout=ss.str();
if (!std::filesystem::exists(fnout)) break;
} // true

return 0; // NonExist(fnout,fnin,flags); 
} // NonExist
