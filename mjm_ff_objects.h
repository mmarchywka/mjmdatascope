...
// no idea why missing z defaults to 1? 
// curently sending mesh data on this line which needs to know about
// defaults lol. 
// apparently the "z" is really a label which defaults to 1 
...
public:
_dof_stats() {Init_dof_stats(); } 
~_dof_stats() {Free_dof_stats(); } 

_dof_stats(const Line & l,  const IdxTy first ) { Init_dof_stats(l,first); }
_dof_stats(const StrTy & s,  const IdxTy flags ) { Init_dof_stats(s,flags); }
_dof_stats(const Ragged & r, const IdxTy first,const IdxTy i0, const IdxTy flags ) 
{Init_dof_stats(r,first,i0,flags); } 

MM_GETSET(D,max,m_max) 
MM_GETSET(D,min,m_min) 
MM_GETSET(D,avg,m_avg) 
MM_GETSET(IdxTy,n,m_n) 
void clear() {Init_dof_stats(); } 
void add_stat(const D pt)
{
if (m_n==0) { m_n=1; m_min=pt; m_max=pt; m_sum=pt; m_avg=pt; return; } 
++m_n; 
if (pt<m_min) m_min=pt;
if (pt>m_max) m_max=pt;
m_sum+=pt;
m_avg=m_sum/m_n; // don're really want to do this each add doh 
}

StrTy save( const IdxTy flags=0) const { return Save_dof_stats(flags); } 
StrTy dump( const IdxTy flags=0) const { return Dump_dof_stats(flags); } 
private:
void Init_dof_stats(const StrTy & s,  const IdxTy flags ) 
{ Init_dof_stats(); 
BaseParams kvp(s);

}
void Init_dof_stats(const Line & l,  const IdxTy first ) 
{ 
Init_dof_stats();
...
void Init_dof_stats(const Ragged & r, const IdxTy first,const IdxTy i0,const IdxTy flags ) 
{
Init_dof_stats();
...
StrTy Save_dof_stats( const IdxTy flags=0) const  
{
StrTy s;
//BaseParams kvp();
BaseParams kvp;
...
StrTy Dump_dof_stats( const IdxTy flags=0) const  
{
Ss ss;
// ss<<MMPR4(); 
return ss.str(); 
...
// _dof_statsMEMBERS
D m_min,m_max,m_sum,m_avg;
IdxTy m_n; 
}; // _dof_stats

typedef _dof_stats DofStats;
...
const DofStats & stats() const { return m_stats; } 
void find_stats()
{
m_stats.clear(); 
MM_LOOP(ii,m_vals){ m_stats.add_stat(*ii); } 
}
...
void push_back(const D & v) { m_vals.push_back(v); } 
// these are all public but wtf
const StrTy & name() const { return m_name; } 
void  name(const StrTy & n )  {  m_name=n; } 
